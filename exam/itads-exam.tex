...
\question
\label{q:sequence-alignment}

\emph{something with sequence alignment\ldots}

\question
\label{q:graph-search}

\emph{something with dfs, bfs, and dijkstra, maybe throw in an adjacency matrix for good measure\ldots}

\question
\label{q:tree-exercises}

\emph{various little things about trees and search trees, e.g.\ draw diagram of binary search tree after a series of insertions and deletion, list the node visitation order for the various traversal methods, find formula for max number of nodes in a ternary tree (proof by induction)\ldots}
