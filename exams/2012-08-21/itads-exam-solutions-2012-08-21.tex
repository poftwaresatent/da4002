...
\item
  ...
  \fbox{\begin{minipage}[t]{0.4\columnwidth}
      ...
      \begin{tabbing}
        ...
        \}\\
        \\
        void move (int size, char src, char dst) \{\\
        \>if (1 $>$= size) \{\\
        \>\>System.out.println ("disk 1 from " + src + " to " + dst);\\
        \>\>return;\\
        \>\}\\
        \>move(size-1, src, other(src, dst));\\
        \>System.out.println ("disk " + size + " from " + src + " to " + dst);\\
        \>move(size-1, other(src, dst), dst);\\
        \}
      \end{tabbing}
  \end{minipage}}
\item
  \emph{(1 point)} for step $i$ and/or stack size $i$: $N_i = 2N_{i-1}+1$ with $N_1=1$.
