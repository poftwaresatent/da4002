...
% just reuse question counter for answer command
\newcommand{\answer}{\subsection*{Answer~\thequestion}}
...
\begin{center}
  \begin{tabular}{|l|p{0.3\columnwidth}|l|}
    \hline
    \emph{algorithm} & \emph{A, B, C, D, or X} & \textbf{answer} \\
    \hline
    linear search & & B\\
    \hline
    binary search & & X\\
    ...
    sort & & A\\
    \hline
    pre-order tree traversal & & D\\
    \hline
    level-order tree traversal & & X\\
    \hline
    breadth-first graph search & & C\\
    \hline
  \end{tabular}
\end{center}
...
\item
  \emph{(1 point)} brute-force method, e.g.
  \fbox{\begin{minipage}[t]{0.4\columnwidth}
    \small
    \begin{tabbing}
      \quad\=\quad\=\quad\=\quad\=\quad\=\kill
      boolean doesitsum (Array$<$int$>$ aa, int sum) \{\\
      \>for (int ii = 0; ii $<$ aa.length - 1; ++ii)\\
      \>\>for (int jj = ii + 1; jj $<$ aa.length; ++jj)\\
      \>\>\>if (ii != jj)\\
      \>\>\>\>if (aa[ii] + aa[jj] == sum)\\
      \>\>\>\>\>return true;\\
      \>return false;\\
...
\item
  \emph{(1 point)} two nested for-loops with linear execution time each $\leadsto O(N^2)$ (it's the classical triangular array traversal case, with $N(N-1)/2$ operations).
    ...
    \item
      ...
      \fbox{\begin{minipage}[t]{0.4\columnwidth}
          ...
          \begin{tabbing}
            \quad\=\quad\=\quad\=\quad\=\quad\=\kill
            sort(aa);\\
            for (ii = 0; ii $<$ aa.length - 2; ++ii) \{\\
            \>int jj = binsearch(aa, sum - ii);\\
            \>if (jj $>$= 0 \&\& jj != ii)\\
            \>\>return true;\\
            \}\\
            return false;
...
\begin{enumerate}
\item
  \emph{(1 point)} the trick is to leverage the call stack, and actually quite simple and directly taken from an exercise, e.g.
  \fbox{\begin{minipage}[t]{0.4\columnwidth}
      \small
      \begin{tabbing}
        \quad\=\quad\=\quad\=\quad\=\quad\=\kill
        void printTernary (int number) \{\\
        \>if (number $>$= 3)\\
        \>\>printTernary (number / 3);\\
        \>System.out.print("" + (number \% 3));\\
        \}
        \end{tabbing}
  \end{minipage}}\\
  okay, this fails to print a newline, but one can always add that by wrapping from another function.
\item
  \emph{(2 points)} this is linear because of the definition via the number of decimal digits (as opposed to e.g.\ the number itself).
  \begin{align*}
  N &= \text{number of decimal digits}\\
  \text{thus, } X &= 10^N\\
  M &= \text{number of ternary digits}\\
  M &= \log_3 X = \frac{\log_{10} X}{\log_{10} 3}\\
  &= \frac{\log_{10} 10^N}{\text{constant}} = \frac{N}{\text{constant}}\\
  \Rightarrow M(N) &\in O(N)
  \end{align*}
\end{enumerate}
...
\begin{enumerate}
\item
  \emph{(2 points)} the essential insights is to first move $N-1$ disks to the other rod, then move the $N^\text{th}$ disk to the desired rod, then move $N-1$ disks from the other rod to the desired rod.\\
  \fbox{\begin{minipage}[t]{0.4\columnwidth}
      \small
      \begin{tabbing}
        \quad\=\quad\=\quad\=\quad\=\quad\=\kill
        void move (int size, char src, char tmp, char dst) \{\\
        \>if (1 $>$= size) \{\\
        \>\>System.out.println ("disk 1 from " + src + " to " + dst);\\
        \>\>return;\\
        \>\}\\
        \>move(size-1, src, tmp);\\
        \>System.out.println ("disk " + size + " from " + src + " to " + dst);\\
        \>move(size-1, tmp, dst);\\
        \}
      \end{tabbing}
  \end{minipage}}\\
  It is also possible to pass two \texttt{char} arguments to the recursive function, but then some sort of utility function is needed to determine the temporary rod.
\item
  \emph{(1 point)} recursive definition: for step $i$ and/or stack size $i$: $N_i = 2N_{i-1}+1$ with $N_1=1$.
  It is crucial to specify the $N_1$ case explicitly, because it terminates the recursion!
  This recursion can be solved and leads to the nice and simple $N_i = 2^N-1,$ but students get full points even if they only give the recursive formula, as long as they give the terminating condition.
  Providing the full proof (by induction) deserves a bonus point.
\end{enumerate}
...
\begin{enumerate}
\item
  \emph{(1 point)}\fbox{\begin{minipage}[t]{4cm}\includegraphics[width=4cm]{answer-5-1}\end{minipage}}
\item
  \emph{(1 point)} the result is just 1, 2, 3, 4, 5, 6, 7, 8, 9 \ldots \textbf{UNFINISHED}
\end{enumerate}
