...
You can use pseudo-code, C, or plain English.
...
\item
  ...
  The resulting linked list must be ordered along X (from lowest to highest X coordinate) and contain all the points A, B, C, D, and E.
...
\item
  Using pseudo-code, C, or plain English, write a procedure to insert a point into such a sorted multilist.
...
\emph{Notice the table of numeric values for $x^2$, $x^3$, and $2^x$ on the bottom of this page, to help you with the computations.}
...
\noindent
You are asked to align the sequences ``violet-blue-red'' (VBR) and ``yellow-orange-green'' (YOG).
